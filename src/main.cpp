...
int main(){
	...
	auto loader = std::make_unique<npc_loader::TextFileLoader>("save.txt");
	npc_factory::NPCFactory factory(std::move(loader));	
	auto cout_observer = std::make_shared<fight_system::fight_observers::CoutObserver>();
	auto log_observer = std::make_shared<fight_system::fight_observers::LogObserver>("fighting_scroll.log");
